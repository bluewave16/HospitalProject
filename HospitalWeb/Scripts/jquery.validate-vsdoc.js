...
$.extend($.fn, {
	...
	rules: function(command, argclassmanagerent) {
		/// <sclassmanagermary>
		...
		/// </param>
		/// <param name="argclassmanagerent" type="">
		...
		if (command) {
			...
			switch(command) {
			case "add":
				$.extend(existingRules, $.validator.normalizeRule(argclassmanagerent));
				staticRules[element.name] = existingRules;
				if (argclassmanagerent.messages)
					settings.messages[element.name] = $.extend( settings.messages[element.name], argclassmanagerent.messages );
				break;
			case "remove":
				if (!argclassmanagerent) {
					delete staticRules[element.name];
					return existingRules;
				}
				var filtered = {};
				$.each(argclassmanagerent.split(/\s/), function(index, method) {
...
$.validator.format = function(source, params) {
	/// <sclassmanagermary>
	/// Replaces {n} placeholders with argclassmanagerents.
	/// One or more argclassmanagerents can be passed, in addition to the string template itself, to insert
	/// into the string.
	/// </sclassmanagermary>
	...
	/// The first argclassmanagerent to insert, or an array of Strings to insert
	/// </param>
	/// <returns type="String" />

	if ( argclassmanagerents.length == 1 ) 
		return function() {
			var args = $.makeArray(argclassmanagerents);
			args.unshift(source);
			return $.validator.format.apply( this, args );
		};
	if ( argclassmanagerents.length > 2 && params.constructor != Array  ) {
		params = $.makeArray(argclassmanagerents).slice(1);
	}
...
$.extend($.validator, {
	...
	messages: {
		...
		nclassmanagerber: "Please enter a valid nclassmanagerber.",
		digits: "Please enter only digits.",
		creditcard: "Please enter a valid credit card nclassmanagerber.",
	...
	prototype: {
		...
		element: function( element ) {
			...
			if ( !this.nclassmanagerberOfInvalids() ) {
		...
		nclassmanagerberOfInvalids: function() {
			/// <sclassmanagermary>
			/// Returns the nclassmanagerber of invalid fields.
		...
		// return the first defined argclassmanagerent, allowing empty strings
		findDefined: function() {
			for(var i = 0; i < argclassmanagerents.length; i++) {
				if (argclassmanagerents[i] !== undefined)
					return argclassmanagerents[i];
			}
			return undefined;
		},
		...
		findByName: function( name ) {
			...
			return $(docclassmanagerent.getElementsByName(name)).map(function(index, element) {
	...
	classRuleSettings: {
		...
		dateDE: {dateDE: true},
		nclassmanagerber: {nclassmanagerber: true},
		nclassmanagerberDE: {nclassmanagerberDE: true},
		digits: {digits: true},
	...
	normalizeRules: function(rules, element) {
		...
		// clean nclassmanagerber parameters
		$.each(['minlength', 'maxlength', 'min', 'max'], function() {
			if (rules[this]) {
				rules[this] = Nclassmanagerber(rules[this]);
			}
		});
		$.each(['rangelength', 'range'], function() {
			if (rules[this]) {
				rules[this] = [Nclassmanagerber(rules[this][0]), Nclassmanagerber(rules[this][1])];
			}
		});
	...
	methods: {
		...
		// http://docs.jquery.com/Plugins/Validation/Methods/nclassmanagerber
		nclassmanagerber: function(value, element) {
	...
	if ( $.ajaxPrefilter ) {
		...
		$.ajax = function(settings) {
			...
			if (mode == "abort") {
			    ...
			    return (pendingRequests[port] = ajax.apply(this, argclassmanagerents));
		    }
		    return ajax.apply(this, argclassmanagerents);
	    };
    }
})(jQuery);
...
;(function($) {
	...
	if (!jQuery.event.special.focusin && !jQuery.event.special.focusout && docclassmanagerent.addEventListener) {
		$.each({
			focus: 'focusin',
			blur: 'focusout'	
			...
			$.event.special[fix] = {
				...
				handler: function(e) {
					argclassmanagerents[0] = $.event.fix(e);
					argclassmanagerents[0].type = fix;
					return $.event.handle.apply(this, argclassmanagerents);
				}
			};
			function handler(e) {
				e = $.event.fix(e);
		...
		validateDelegate: function(delegate, type, handler) {
			return this.bind(type, function(event) {
				...
				if (target.is(delegate)) {
					return handler.apply(target, argclassmanagerents);
				}
			});
		}
	});
